...
pub fn apply_viewport_builder_to_window(
...

    {
        ...
        if let Some(size) = builder.inner_size {
            // The persisted window size can be far below `min_inner_size`
            // (a minimized window is saved as 0x0; eframe then sanity-clamps
            // it to 64x64). winit clamps the size once at window creation,
            // but this later request would bypass that and shrink the window
            // below the app's own minimum — clamp here too, like
            // `winit::dpi::Size::clamp` does.
            let size = builder.min_inner_size.map_or(size, |min| size.max(min));
            let size = builder.max_inner_size.map_or(size, |max| size.min(max));
            if window
